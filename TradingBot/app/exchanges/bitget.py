"""
Bitget Exchange Wrapper - Bitget USDT Perpetual Futures trading.
"""
import sys
import logging
import asyncio
...
from ..config import (
    BITGET_API_KEY, BITGET_API_SECRET, BITGET_PASSPHRASE, DRY_RUN, EXCHANGE_TIMEOUT_MS, 
    EXCHANGE_RETRIES, RETRY_DELAY_MULTIPLIER
)


class BitgetExchange(ExchangeBase):
    """Wrapper for Bitget USDT Perpetual Futures trading."""
    
    def __init__(self, config=None):
        """Initialize Bitget exchange wrapper."""
        self.logger = get_logger("Bitget")
        self.config = config
        self.exchange = None
        self.markets = {}
        ...
        if config:
            self.api_key = getattr(config, 'BITGET_API_KEY', BITGET_API_KEY)
            self.api_secret = getattr(config, 'BITGET_API_SECRET', BITGET_API_SECRET)
            self.passphrase = getattr(config, 'BITGET_PASSPHRASE', BITGET_PASSPHRASE)
    ...
    async def initialize(self):
        ...
        options = {
            ...
            "options": {
                "defaultType": "swap",  # Bitget uses "swap" for perpetuals
            }
        }
        ...
        if self.api_key and self.api_secret and self.passphrase:
            ...
            options["password"] = self.passphrase  # Bitget uses "password" for passphrase
        
        self.exchange = ccxt_async.bitget(options)
        
        if self.testnet:
            try:
                self.exchange.set_sandbox_mode(True)
                self.logger.info("Bitget testnet mode enabled")
        ...
        for i in range(1, attempts + 1):
            try:
                ...
                self.logger.info(f"Bitget initialized: {len(self.markets)} markets loaded")
                return
            except Exception as e:
                last_err = e
                self.logger.warning(f"Bitget market load attempt {i}/{attempts} failed: {type(e).__name__}: {str(e)}")
                if i < attempts:
                    await asyncio.sleep(RETRY_DELAY_MULTIPLIER * i)
        
        if last_err:
            self.logger.error(f"Bitget initialization failed after {attempts} attempts: {last_err}")
            raise last_err
    ...
    def denormalize_symbol(self, symbol: str) -> str:
        """Denormalize symbol format for Bitget API."""
        if symbol in self._denormalize_cache:
            return self._denormalize_cache[symbol]
        
        # Bitget uses "BTC/USDT:USDT" format for perpetuals
        if "/" in symbol:
            parts = symbol.split("/")
            base = parts[0].upper()
