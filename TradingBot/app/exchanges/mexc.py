"""
MEXC Exchange Wrapper - MEXC USDT Perpetual Futures trading.
"""
import sys
import logging
import asyncio
...
from ..config import (
    MEXC_API_KEY, MEXC_API_SECRET, DRY_RUN, EXCHANGE_TIMEOUT_MS, 
    EXCHANGE_RETRIES, RETRY_DELAY_MULTIPLIER
)


class MEXCExchange(ExchangeBase):
    """Wrapper for MEXC USDT Perpetual Futures trading."""
    
    def __init__(self, config=None):
        """Initialize MEXC exchange wrapper."""
        self.logger = get_logger("MEXC")
        self.config = config
        self.exchange = None
        self.markets = {}
        ...
        if config:
            self.api_key = getattr(config, 'MEXC_API_KEY', MEXC_API_KEY)
            self.api_secret = getattr(config, 'MEXC_API_SECRET', MEXC_API_SECRET)
            self.dry_run = getattr(config, 'DRY_RUN', DRY_RUN)
            self.testnet = getattr(config, 'MEXC_TESTNET', False)
        else:
    ...
    async def initialize(self):
        ...
        options = {
            ...
            "options": {
                "defaultType": "swap",  # MEXC uses "swap" for perpetuals
            }
        }
        ...
        self.exchange = ccxt_async.mexc(options)
        
        if self.testnet:
            try:
                self.exchange.set_sandbox_mode(True)
                self.logger.info("MEXC testnet mode enabled")
        ...
        for i in range(1, attempts + 1):
            try:
                ...
                self.logger.info(f"MEXC initialized: {len(self.markets)} markets loaded")
                return
            except Exception as e:
                last_err = e
                self.logger.warning(f"MEXC market load attempt {i}/{attempts} failed: {type(e).__name__}: {str(e)}")
                if i < attempts:
                    await asyncio.sleep(RETRY_DELAY_MULTIPLIER * i)
        
        if last_err:
            self.logger.error(f"MEXC initialization failed after {attempts} attempts: {last_err}")
            raise last_err
    ...
    def denormalize_symbol(self, symbol: str) -> str:
        """Denormalize symbol format for MEXC API."""
        if symbol in self._denormalize_cache:
            return self._denormalize_cache[symbol]
        
        # MEXC uses "BTC/USDT:USDT" format for perpetuals
        if "/" in symbol:
            parts = symbol.split("/")
            base = parts[0].upper()
